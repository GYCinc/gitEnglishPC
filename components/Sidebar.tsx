...
const VocabularyFocus: React.FC<VocabularyFocusProps> = ({ focusVocabulary, setFocusVocabulary, inclusionRate, setInclusionRate }) => {
    ...
    return (
        <div className="mb-4 bg-slate-900/50 rounded-xl ring-1 ring-slate-700 overflow-hidden">
                    ...
                    <div>
                        <div className="flex gap-2">
                            <input
                                ...
                                className="w-full bg-slate-800 text-slate-200 border border-slate-600 rounded-md shadow-sm px-3 py-1.5 text-xs focus:ring-1 focus:ring-yellow-500 focus:border-yellow-500 focus:outline-none"
                            />
                            <button onClick={handleAddVocab} className="bg-yellow-500 text-slate-900 font-bold p-1.5 rounded-md hover:bg-yellow-400 transition-colors">
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                                {focusVocabulary.map(v => (
                                    <span key={v} className="flex items-center bg-yellow-500/10 text-yellow-300 text-xs font-medium px-2.5 py-1 rounded-full border border-yellow-500/20">
                                        {v}
                                        <button onClick={() => handleRemoveVocab(v)} className="ml-1.5 text-yellow-400 hover:text-white">
...
const GrammarFocus: React.FC<GrammarFocusProps> = ({ focusGrammar, setFocusGrammar, grammarInclusionRate, setGrammarInclusionRate }) => {
    ...
    return (
        <div className="mb-6 bg-slate-900/50 rounded-xl ring-1 ring-slate-700 overflow-hidden">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center p-4 text-left bg-slate-800/50 hover:bg-slate-800 transition-colors">
                <div className="flex items-center">
                ...
                <div className="p-3 border-t border-slate-700 space-y-4 bg-slate-900/30">
                    <div>
                        <div className="flex gap-2">
                            ...
                            />
                             <button onClick={handleAddGrammar} className="bg-emerald-500 text-slate-900 font-bold p-1.5 rounded-md hover:bg-emerald-400 transition-colors">
                                <PlusIcon className="w-4 h-4" />
                            </button>
                        </div>
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                                {focusGrammar.map(g => (
                                    <span key={g} className="flex items-center bg-emerald-500/10 text-emerald-300 text-xs font-medium px-2.5 py-1 rounded-full border border-emerald-500/20">
                                        {g}
                                        <button onClick={() => handleRemoveGrammar(g)} className="ml-1.5 text-emerald-400 hover:text-white">
                                            <XCircleIcon className="w-3.5 h-3.5"/>
                                        </button>
                                    </span>
                                ))}
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-40 w-80 bg-slate-900 text-white flex flex-col h-screen transition-transform duration-300 ease-in-out border-r border-slate-800 shadow-2xl
                     lg:static lg:translate-x-0
                     ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      
      <div className="p-6 pb-4 flex-shrink-0 border-b border-slate-800 bg-slate-900 z-10">
        <h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-orange-400 to-yellow-400">gitEnglish™</h1>
        <h2 className="text-sm font-medium text-slate-400 mt-0.5 tracking-wide uppercase">Practice Genie</h2>
      </div>
      ...
      <div className="flex-grow overflow-y-auto custom-scrollbar-dark p-4">
          ...
          <div className="mb-6">
              <button 
                ...
                className="flex items-center justify-between w-full p-2 mb-2 text-xs font-bold text-slate-500 uppercase tracking-wider hover:text-slate-300 transition-colors"
              >
          ...
          <div className="pb-8">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 ml-3">Exercise Library</h3>
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-950 text-center text-xs text-slate-600">
